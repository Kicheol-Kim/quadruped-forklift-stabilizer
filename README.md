...
* **`main.py` (전체 시스템 통합 오케스트레이터):**
  ...
  * (구현 예정 내용) Go1과 stabilizer의 독립적인 제어명령을 바탕으로 물체를 들어올리고 내리는 전체 시스템의 제어명령을 취급합니다.
* **`Go1_controller.py` (사용자 인터페이스 및 명령 송신 노드):**
  * Tkinter 기반의 GUI 환경으로 오직 'Go1 제어 및 명령 생성'에만 집중하는 논블로킹(Non-blocking) 노드입니다.
  * 사용자가 입력한 고위 주행 명령(속도, 주행거리, 선회반경)을 실시간으로 계산하여 주행 시간 주기를 도출합니다.
...
* **`data_analysis.py` (구현 예정):**
  * 수집한 데이터들을 바탕으로 가감속 구간 가속도와 정속주행중 stabilizer 모듈에 발생하는 노이즈 등을 분석
