...
// 📡 [Wi-Fi 및 네트워크 설정 추가]
char ssid[] = "rml-kkch";           // 노트북 핫스팟 이름
char pass[] = "00000000";           // 노트북 핫스팟 비밀번호
IPAddress pcIP(192, 168, 50, 1);    // 노트북(파이썬) IP 주소
unsigned int pcPort = 9998;         // 파이썬 수신 대기 포트
unsigned int localPort = 8888;      // 스태빌라이저 로컬 포트
int status = WL_IDLE_STATUS;
WiFiUDP Udp;

unsigned long lastSendTime = 0;
const int sendInterval = 10;        // UDP 송신 주기 (10ms = 100Hz)
...
// ==========================================
// 🧠 코어 0: IMU 연산, 독립 튜닝 인터페이스 처리 및 무선 전송
// ==========================================
void setup() {
  Serial.begin(115200);
  delay(2000); 
...
void loop() {
  ...
  // 2. 1초마다 현재 튜닝 상태 출력 (원본 유지)
  if (millis() - last_print_time >= 1000) {
    last_print_time = millis();
    Serial.print("🛠️ Roll  [P: "); Serial.print(hw_p_gain_roll); Serial.print(" / D: "); Serial.print(hw_d_gain_roll); Serial.print("]  |  ");
    Serial.print("Pitch [P: "); Serial.print(hw_p_gain_pitch);
    Serial.print(" / D: "); Serial.print(hw_d_gain_pitch); Serial.print("]  |  ");
  ...
  if (IMU.accelerationAvailable()) {
    ...
    // 📡 [Wi-Fi 무선 전송 추가] - 10ms(100Hz) 마다 파이썬으로 전송
    if (millis() - lastSendTime >= sendInterval) {
      lastSendTime = millis();
      
      Udp.beginPacket(pcIP, pcPort);
      Udp.print("STAB,");
      Udp.print(imu_roll, 3);
      Udp.print(",");
      Udp.print(imu_pitch, 3);
      Udp.endPacket();
    }
  }
